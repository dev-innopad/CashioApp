...
export const generatePrimaryColors = (
  ...
  return Object.fromEntries(
    Object.entries(lightenLevels).map(([key, amt]) => [
      key,
...
export const generateTransparentPrimaryColors = (
  baseColor: string,
): Record<string, string> => {
  ...
  return Object.fromEntries(
    Object.entries(transparencyLevels).map(([key, alpha]) => {
      ...
      return [key, `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${alpha})`];
    }),
  );
};

// Utility: Generate secondary colors depending on theme mode ('light' or 'dark')
export const generateSecondaryColors = (
  baseColor: string,
  mode: 'light' | 'dark',
): Record<string, string> => {
  const levels =
  ...
  return Object.fromEntries(
    Object.entries(levels).map(([key, amt]) => [
      key,
      tinycolor(baseColor).lighten(amt).toRgbString(),
    ]),
  );
...
export const generateTheme = (baseColor: string, mode: 'light' | 'dark') => {
  ...
  const TRANSPARENT_PRIMARY_COLORS =
    generateTransparentPrimaryColors(baseColor);
  ...
  return {
    ...
    backgroundTransparent:
      mode === 'light' ? 'rgba(255, 255, 255, 0.8)' : 'rgba(0, 0, 0, 0.8)',

    textColor:
      mode === 'light' ? 'rgba(11, 11, 11, 1)' : 'rgba(255, 255, 255, 1)',

    segmentBackground:
      mode === 'light' ? 'rgba(230, 230, 230, 1)' : 'rgba(35, 35, 35, 1)',
...
export const gradientColors = ['#141326', '#4A1622'];
