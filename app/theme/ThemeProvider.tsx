...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children, isDisabled = false }) => {
	...
	const AppColors = useMemo(() => {
		...
		switch (themeId) {
			case THEME_ID_SYSTEM:
				return generateTheme(themeColor, systemColorScheme === 'dark' ? 'dark' : 'light');
			case THEME_ID_LIGHT:
	...
	const barStyleCondition = useMemo<StatusBarStyle>(() => {
		...
		const effectiveTheme = themeId === THEME_ID_SYSTEM
			? systemColorScheme // Use system theme
			: themeId === THEME_ID_LIGHT ? 'light' : 'dark'; // Use light/dark theme
	...
	const toggleTheme = () => {
		const newThemeId = themeId === THEME_ID_LIGHT ? THEME_ID_DARK : THEME_ID_LIGHT;
